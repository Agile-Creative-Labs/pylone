...
class ChatHandler:
    ...
    def correct_input(self, message: str) -> str:
        """
        Corrects the user's input using a spell checker.
    ...
    def collect_project_charter_info(self, user_id: str, message: str) -> str:
        ...
        if current_step == "Project Name":
            ...
            state["project_charter"]["Stakeholders"] = self.extract_relevant_info(message, "Stakeholders")
    ...
    def extract_relevant_info(self, message: str, field: str) -> str:
        """
        Extracts the relevant information from the user input based on the field.

        Args:
            message (str): The user's input message.
            field (str): The field being processed (e.g., "Project Name", "Objectives").

        Returns:
            str: The extracted relevant information.
        """
        # Remove unnecessary phrases like "project name is", "objectives is", etc.
        if field == "Project Name":
            return message.replace("project name is", "").strip()
        elif field == "Objectives":
            return message.replace("objectives is", "").strip()
        elif field == "Scope":
            return message.replace("scope is", "").strip()
        elif field == "Deliverables":
            return message.replace("deliverables are", "").strip()
        elif field == "Timeline":
            return message.replace("timeline is", "").strip()
        elif field == "Stakeholders":
            return message.replace("stakeholders are", "").strip()
        else:
    ...
    def generate_project_charter(self, user_inputs: Dict[str, str]) -> str:
        ...
        formatted_charter = template.format(**user_inputs)
